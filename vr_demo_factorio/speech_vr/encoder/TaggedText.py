...
class TaggedText():
    ...
    def __contains__(self, item):
        typ = type(item)
        if typ is list:
            return any([it in self for it in item])
        elif typ is tuple:
            return all([it in self for it in item])
        elif typ is str:
            return self.contains_pos_token(item)
        else:
            return self.contains_tagged_token(item)
    ...
    def contains_pos_token(self, token: str, pos_str: str = "*", case_sensitive=False, include_subcategories: bool = True):
        ...
        """
        return self.contains_tagged_token(token=token,
                                          case_sensitive=case_sensitive, include_subcategories=include_subcategories)

    def contains_tagged_token(self, token: str, case_sensitive: bool = False, include_subcategories: bool = True):
        """
        ...
        for a in self.tokens:
            if self.token_equals(a, token, case_sensitive):
                return True
        return False
